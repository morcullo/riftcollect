...
async function json(url){const r=await fetch(url,{headers:{"User-Agent":UA,"Accept":"application/json"}});if(!r.ok)throw Error(`TCGCSV HTTP ${r.status}`);return r.json()}
...
async function loadProducts(){
 ...
 const g=(await json(`${BASE}/${CATEGORY}/groups`)).results||[];
 const products=[];
 ...
 cache.groups=g;cache.products=products;return cache;
}
